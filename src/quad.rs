...
impl Pipeline {
    ...
    pub fn draw(
        ...
        while i < total {
            ...
            let quads = instances[i..end].iter().map(|i| i.0).collect::<Vec<Quad>>();
...

            {
                ...
                render_pass.set_scissor_rect(
                    instances[i].1.x0 as u32,
                    instances[i].1.y0 as u32,
                    instances[i].1.width() as u32,
                    instances[i].1.height() as u32,
                );
